...
An independent homage to **Elite** (Bell & Braben, 1984) — the game that put eight
galaxies, 2 048 star systems and an open-ended trading career into 22 kilobytes. This is a
work-in-progress remake with textured 3-D, built on the original's own algorithms: the same
seed produces the same universe, so Lave is exactly where you left it in 1984.
...
You start docked at Lave with a Cobra Mk III, 100 credits, a pulse laser and a hold full of
...
Your combat kills push you up the same rating ladder as the original — Harmless, Mostly
Harmless, Poor, Average, and on up. Almost nobody ever reaches **Elite**.
...
is as unforgiving as it was in 1984, which is the point.

### Screens

The number keys are the original's, and they mean the same screen whether you are docked or
paused in flight — the four that need a station simply do nothing out in space.
...
oscillators and noise, the same ingredients the 1984 machines had.
...
**The market at Lave.** Prices and available quantities come from the original's price
algorithm; the starred goods are the ones that will get you arrested. Station screens paint
...
procedurally generated description — all derived from the system's seed, exactly as in 1984.
...
*Elite* and its universe belong to their original authors and Frontier Developments. This is
an independent fan homage, made with no claim to that IP and not affiliated with or endorsed
by the rights holders. Nothing was extracted from any original binary: the universe
generation follows the co-author's freely published `txtelite` reference and the source
archaeology at [bbcelite.com](https://www.bbcelite.com/).

Original code © the author, all rights reserved — see [LICENSE](LICENSE).
3-D rendering by [Three.js](https://threejs.org/) (MIT).
