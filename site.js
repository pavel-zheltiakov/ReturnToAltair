...
(function () {
    ...
    // Every build there is, grouped the way somebody looks for one: operating system first, then
    // chip. One row per OS rather than one per file, because a row per file says "Windows" twice
    // and "Linux" twice, and the name a reader is scanning for is then the least distinct thing
    // on the line. The extension is named once at the end of each row — it does not vary by chip.
    //
    // macOS is one entry for both chips: the bundle is universal, so an Apple silicon row and an
    // Intel row would be the same download listed twice under two names.
    var BUILDS = [
        { platform: 'macos', name: 'macOS', ext: 'dmg', arches: [
            { arch: 'universal', label: 'arm64+x64' } ] },
        { platform: 'windows', name: 'Windows', ext: 'zip', arches: [
            { arch: 'x64', label: 'x64' }, { arch: 'arm64', label: 'arm64' } ] },
        { platform: 'linux', name: 'Linux', ext: 'AppImage', arches: [
            { arch: 'x64', label: 'x64' }, { arch: 'arm64', label: 'arm64' } ] }
    ];
    ...
    function asset(release, build) {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            if (build.platform === 'linux') {
                // Linux artifacts are named with uname's architectures — x86_64 and aarch64 —
                // and not with the x64/arm64 the rest of this file speaks. That is not a slip in
                // package.sh to be tidied up: appimagetool's ARCH must be exactly those strings,
                // and the AppImage tooling expects to find them in the filename too. So the
                // translation belongs here, at the one place that reads the names.
                //
                // Getting this wrong is quiet. Both AppImages published and the page showed
                // Linux greyed out with no size, which reads as "not built for Linux yet" rather
                // than as a failed lookup — the same page had just listed macOS and Windows.
                var uname = build.arch === 'arm64' ? 'aarch64' : 'x86_64';
                ...
                if (name.indexOf('-' + uname + '.AppImage') > 0) return files[i];
                if (name.indexOf('-linux-' + uname + '.tar.gz') > 0) return files[i];
            }
        }
    ...
    // ---- the version badge ---------------------------------------------------------------------
    //
    // On every page, and from the same release the download panel reads, so there is one answer to
    // "which version is this" and no file to remember to edit. Until GitHub answers — or if it
    // never does — the badge keeps the phase it was written with, which is true whatever the
    // newest tag turns out to be.
    ...
    (function () {
        ...
        newest.then(function (all) {
            ...
            // Without the leading "v": the badge is a label rather than a git ref, and the tag is
            // named in full on the panel and on the releases page.
            var tag = String(all[0].tag_name || '').replace(/^v/, '');
            if (!tag) return;

            Array.prototype.forEach.call(badges, function (badge) {
                badge.textContent = tag + ' · ' + badge.textContent.trim();
            });
        });
    })();
    ...
    (function () {
        ...
        // Three lines, under the notes and small. A reader who has come this far knows which one is
        // theirs; this is a reference table, not the offer — the offer is the picker on the front
        // page. An OS the release has not got keeps its line and goes quiet, so the list is always
        // the same three lines and a gap is visible as a gap rather than as an absence.
        function assets(release) {
            var rows = BUILDS.map(function (os) {
                var found = [];

                var chips = os.arches.map(function (spec) {
                    var file = asset(release, { platform: os.platform, arch: spec.arch });

                    // Unlinked when the release has no such file. Still named, because "Windows
                    // (x64, arm64)" with one of them dead says what is coming; dropping it says
                    // only that this row is short, and leaves the reader to guess why.
                    if (!file) return escape(spec.label);

                    found.push(file);
                    return '<a href="' + escape(file.browser_download_url) + '" title="' +
                        escape(file.name) + '">' + escape(spec.label) + '</a>';
                }).join(', ');

                // One size per file present, in the order the chips are listed. Two sizes on a row
                // read left to right against the two chips above them.
                var size = found.length
                    ? found.map(function (f) { return megabytes(f.size); }).join(' / ')
                    : '—';

                return '<li><span class="dl"><span class="dl-os">' + escape(os.name) + '</span> (' +
                    chips + ', ' + escape(os.ext) + ')</span><span>' + escape(size) + '</span></li>';
            }).join('');
